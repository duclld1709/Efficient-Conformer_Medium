...
def load_datasets(training_params, tokenizer_params, args):
    ...
    mode_key = args.mode.split("-")[0]

    training_entry = training_datasets[training_params["training_dataset"]]
    evaluation_entry = evaluation_datasets[training_params["evaluation_dataset"]]
    training_dataset = training_entry["class"]
    evaluation_dataset = evaluation_entry["class"]

    training_split = training_entry["split"].get("training") if mode_key == "training" else None
    eval_split_key = mode_key if mode_key in evaluation_entry["split"] else "training"
    evaluation_split = evaluation_entry["split"].get(eval_split_key)
